...
/// This crate has a runtime dependency on artifacts produced by the `xtc-ebpf` crate.
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "xtc-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `xtc-ebpf`, so cargo will automatically rebuild us
        // if `xtc-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `xtc-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("xtc-ebpf");
